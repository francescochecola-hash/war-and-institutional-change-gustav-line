...
#
# Script: 14_rdrobust_datadriven.R
# Purpose:
...
# Output:  results/tables/table3_rdrobust_datadriven.csv
# Notes:
...
out_csv     <- file.path(tables_dir, "table3_rdrobust_datadriven.csv")
...
# scale y to percentage points if 0-1
...
# support check
if (sum(df$x < 0) == 0 || sum(df$x > 0) == 0) stop("No support on one side of the cutoff.")
...
# ------------------------------------------------------------------------------
# Extract robust inference from rdrobust object (no summary parsing)
# ------------------------------------------------------------------------------
...
run_rd <- function(d, with_covariates = FALSE, bwselect = c("mserd", "cerrd")) {
  ...
  if (with_covariates) {
    ...
    rd <- rdrobust(y = dd$y, x = dd$x, c = 0, covs = cov_mat, bwselect = bwselect)
  } else {
    dd <- d
    rd <- rdrobust(y = dd$y, x = dd$x, c = 0, bwselect = bwselect)
  }
...
# ------------------------------------------------------------------------------
res <- dplyr::bind_rows(
  run_rd(df, with_covariates = FALSE, bwselect = "mserd"),
  run_rd(df, with_covariates = TRUE,  bwselect = "mserd"),
  run_rd(df, with_covariates = FALSE, bwselect = "cerrd"),
  run_rd(df, with_covariates = TRUE,  bwselect = "cerrd")
) %>%
  mutate(
  ...
  select(spec, bwselect, N_used, tau, se, p, ci95_l, ci95_u, h_l, h_r) %>%
