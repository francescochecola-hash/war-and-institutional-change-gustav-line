...
Scripts are numbered to reflect the recommended execution order of the replication workflow.

The entire replication pipeline can be executed using:

- `00_run_all.R` – runs all scripts sequentially in the correct order.

Alternatively, scripts can be executed step by step following the sequence below.
...
2. Select and prepare analysis variables  
   - `06_select_fontana_et_al_data.R`  
   - `07_select_gagliarducci_et_al_data.R`  
   - `08_select_referendum_1946_data.R`  
   - `09_select_gis_distance_gustav_data.R`

3. Build merged datasets  
   - `10_merge_dataset.R`  
   - `11_merge_dataset_maps.R`
...
- Raw datasets are not included in the repository and are expected to be placed in the `data/raw/` directory before running the scripts.
- All intermediate datasets generated by the scripts are saved in the `data/processed/` directory.
- Tables and figures produced by the analysis are saved in the `results/` directory.
